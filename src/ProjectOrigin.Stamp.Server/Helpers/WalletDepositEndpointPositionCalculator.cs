using System;
using System.Globalization;

namespace ProjectOrigin.Stamp.Server.Helpers;

public static class WalletEndpointPositionCalculator
{
    private static readonly long StartDate = DateTimeOffset.Parse("2022-01-01T00:00:00Z", CultureInfo.InvariantCulture).ToUnixTimeSeconds();

    public static uint? CalculateWalletEndpointPosition(long dateFrom)
    {
        var secondsElapsed = dateFrom - StartDate;

        if (secondsElapsed < 0)
            return null;

        if (secondsElapsed % 60 != 0)
            return null;

        var minutesElapsed = secondsElapsed / 60;
        if (minutesElapsed > int.MaxValue)
            return null;

        return Convert.ToUInt32(minutesElapsed);
    }
}
